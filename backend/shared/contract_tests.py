...
class TestErrorContractCompliance:
    ...
    def validate_error_response(self, response_data: dict[str, Any]) -> bool:
        ...
        assert isinstance(response_data["code"], str) and response_data["code"], (
            "code must be non-empty string"
        )
        assert isinstance(response_data["message"], str) and response_data["message"], (
            "message must be non-empty string"
        )
        assert response_data["request_id"], "request_id must be non-empty"

        # details can be dict, list, or string
        assert isinstance(response_data["details"], (dict, list, str)), (
            "details must be object, list, or string"
        )
    ...
    def test_400_validation_error_contract(self):
        ...
        sample_error = {
            ...
            "details": {"email": ["Invalid email format"], "age": ["Must be >= 0"]},
...
class TestSuccessContractCompliance:
    ...
    def validate_success_response(
        self, response_data: dict[str, Any], must_have_pagination: bool = False
    ) -> bool:
        """
        ...
        if must_have_pagination:
            ...
            assert isinstance(pagination["limit"], int) and pagination["limit"] > 0, (
                "limit must be positive integer"
            )
            assert isinstance(pagination["offset"], int) and pagination["offset"] >= 0, (
    ...
    def test_single_resource_contract(self):
        ...
        sample_response = {
            ...
            "meta": {"request_id": "req-single-456"},
        }
    ...
    def test_empty_list_response_contract(self):
        ...
        assert not sample_response["meta"]["pagination"]["has_next"]
...
class TestDeprecationHeaderCompliance:
    ...
    def validate_deprecation_headers(
        self, headers: dict[str, str], expect_deprecated: bool = True
    ) -> bool:
        """
        ...
        if expect_deprecated:
            ...
            assert any(h in headers for h in ["Sunset", "sunset"]), (
                "Missing Sunset header (death date)"
            )
            assert any(h in headers for h in ["Link", "link"]), (
                "Missing Link header (successor info)"
            )
