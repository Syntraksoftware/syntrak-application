...
class ActivityOperations(SupabaseBase):
    ...
    def create_activity(
        ...
        try:
            payload = {
                ...
                "average_pace": average_pace, "max_pace": max_pace,
                "calories": calories, "is_public": is_public,
            }
    ...
    def get_activity_by_id(self, activity_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            activity = data[0]
            loc_resp = client.table("activity_locations").select("*").eq(
                "activity_id", activity_id
            ).order("point_order").execute()
    ...
    def list_activities_by_user(
        ...
        try:
            resp = client.table("activities").select("*").eq(
                "user_id", user_id
    ...
    def list_public_activities(self, limit: int = 20, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        try:
            resp = client.table("activities").select(
                "*, user_info!activities_user_id_fkey(email, first_name, last_name)"
            ).eq("is_public", True).order("created_at", desc=True).range(
